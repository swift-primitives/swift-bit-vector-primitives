        ...
        public init(capacity: Bit.Index.Count, repeating value: Bool, count: Bit.Index.Count) throws(Self.Error) {
            ...
            if value && count > .zero {
                ...
                if countPack.bits.unused > .zero {
                    // WHY: this branch runs only when count > .zero (outer guard), so countPack.words.count is at least one; the predecessor of that word count is in range and .exact() can never throw.
