        ...
        public init(repeating value: Bool, count: Bit.Index.Count) throws(Self.Error) {
            ...
            if value && count > .zero {
                ...
                if pack.bits.unused > .zero {
                    // WHY: this branch runs only when count > .zero (outer guard), so pack.words.count is at least one; the predecessor of that word count is in range and .exact() can never throw.
...
extension Bit.Vector.Inline {
    ...
    @inlinable
    public func get(_ index: Bit.Index) throws(Bit.Vector.Inline.Error) -> Bool {
