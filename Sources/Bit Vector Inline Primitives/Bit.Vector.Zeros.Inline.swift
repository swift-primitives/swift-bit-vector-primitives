...
extension Bit.Vector.Zeros {
    /// A sequence of clear-bit indices for `Bit.Vector.Inline`.
    ///
    /// Copies the `InlineArray` from the inline vector so iteration does not
    /// alias mutable storage. Unlike `Zeros.Static`, this checks against
...
extension Bit.Vector.Zeros.Inline: Sequence.`Protocol` {
