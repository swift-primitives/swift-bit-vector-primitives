...
extension Bit.Vector.Zeros.Inline {
    ...
    /// An iterator that produces clear-bit indices from an `InlineArray` of words.
    ///
    /// Same complement + Wegner/Kernighan algorithm as `Zeros.Static.Iterator` but
    /// bounds-checks against the logical capacity (Inline has variable count).
    ///
...
extension Bit.Vector.Zeros.Inline.Iterator {
